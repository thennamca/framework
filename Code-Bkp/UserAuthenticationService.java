...
public class UserAuthenticationService implements UserDetailsService {
	...
	private void checkLicence() {
		...
		String line = null;
		fileName = environment.getProperty("licence.path");
		if (StringUtils.isNotBlank(fileName)) {
			try {
				...
				if (!f.exists() && !f.isDirectory()) {
					...
					if (!CommonUtil.macAddressValidate(myMap.get("macAddress"), CommonUtil.getMAC())) {
						throw new ApplicationException(
			...
			throw new ApplicationException("Licence configuration not done Please add file path in application properties");
		}
	...
	public boolean checkDayCount(Map<String, String> myMap) {
		...
		TimerTask task = new TimerTask() {
			@Override
			public void run() {
				String fileName = environment.getProperty("licence.key.path");
				System.out.println("Key File Path::::"+environment.getProperty("licence.key.path"));
				if (StringUtils.isNotBlank(fileName)) {
					int count = 0;
					try {
						File f = new File(fileName);
